import "./App.css";
import "@playground-assets/reason-react-cmijs";
import "@playground-assets/mel_playground.bc.js";
import "@playground-assets/melange-cmijs.js";
import "@playground-assets/format.bc.js";
...
import LanguageToggle, { languageMap } from './LanguageToggle.jsx';
...
function App() {
  ...
  const setLive = (live) => setState(prev => ({ ...prev, live }));
  const setCode = (code) => setState(prev => ({ ...prev, code }));
  const setInput = ({ language, code }) => setState(prev => ({ ...prev, language, code }));
  ...
  const onLanguageToggle = React.useCallback((newLanguage) => {
  ...
  }, [language, code, setInput]);

  // Watch for language changes from the navbar LanguageToggle (via body classList)
  React.useEffect(() => {
    const getLanguageFromClassList = () => {
      if (document.body.classList.contains('syntax__reasonml')) {
        return languageMap.Reason;
      } else if (document.body.classList.contains('syntax__ocaml')) {
        return languageMap.OCaml;
      }
      return null;
    };

    const observer = new MutationObserver(() => {
      const detectedLanguage = getLanguageFromClassList();
      if (detectedLanguage && detectedLanguage !== language) {
        onLanguageToggle(detectedLanguage);
      }
    });
    ...
    const initialLanguage = getLanguageFromClassList();
    if (initialLanguage && initialLanguage !== language) {
      onLanguageToggle(initialLanguage);
    }
