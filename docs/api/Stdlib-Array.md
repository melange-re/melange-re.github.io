...
```
```reasonml
let init_matrix: int => int => (int => int => 'a) => array(array('a));
```
...
```
```reasonml
let equal: ('a => 'a => bool) => array('a) => array('a) => bool;
```
...
```
```reasonml
let compare: ('a => 'a => int) => array('a) => array('a) => int;
```
...
```
```reasonml
let map_inplace: ('a => 'a) => array('a) => unit;
```
...
```
```reasonml
let mapi_inplace: (int => 'a => 'a) => array('a) => unit;
```
...
```
```reasonml
let fold_left: ('acc => 'a => 'acc) => 'acc => array('a) => 'acc;
```
...
```
```reasonml
let fold_right: ('a => 'acc => 'acc) => array('a) => 'acc => 'acc;
```
...
```
```reasonml
let for_all2: ('a => 'b => bool) => array('a) => array('b) => bool;
```
...
```
```reasonml
let find_opt: ('a => bool) => array('a) => option('a);
```
...
```
```reasonml
let find_index: ('a => bool) => array('a) => option(int);
```
...
```
```reasonml
let find_map: ('a => option('b)) => array('a) => option('b);
```
...
```
```reasonml
let find_mapi: (int => 'a => option('b)) => array('a) => option('b);
```
...
let size = 100_000_000;
let a = Array.make(size, 1);
let d1 = Domain.spawn(() => Array.iteri((i, x) => a[i] = x + 1, a));
let d2 = Domain.spawn(() => Array.iteri((i, x) => a[i] = 2 * x + 1, a));
let () = {
  Domain.join(d1);
  Domain.join(d2);
};
