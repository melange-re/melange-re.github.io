...
type scanner('a, 'b, 'c, 'd) =
  ...
  'c;
```
The type of formatted input scanners: <code class="text-ocaml">('a, 'b, 'c, 'd) scanner</code><code class="text-reasonml">scanner('a, 'b, 'c, 'd)</code> is the type of a formatted input function that reads from some formatted input channel according to some format string; more precisely, if `scan` is some formatted input function, then `scan ic fmt f` applies `f` to all the arguments specified by format string `fmt`, when `scan` has read those arguments from the [`Scanning.in_channel`](./Stdlib-Scanf-Scanning.md#type-in_channel) formatted input channel `ic`.

For instance, the [`Scanf.scanf`](./#val-scanf) function below has type <code class="text-ocaml">('a, 'b, 'c, 'd) scanner</code><code class="text-reasonml">scanner('a, 'b, 'c, 'd)</code>, since it is a formatted input function that reads from [`Scanning.stdin`](./Stdlib-Scanf-Scanning.md#val-stdin): `scanf fmt f` applies `f` to the arguments specified by `fmt`, reading those arguments from [`Stdlib.stdin`](./Stdlib.md#val-stdin) as expected.
...
- `r`: user-defined reader. Takes the next `ri` formatted input function and applies it to the scanning buffer `ib` to read the next argument. The input function `ri` must therefore have type <code class="text-ocaml">Scanning.in_channel -&gt; 'a</code><code class="text-reasonml">Scanning.in_channel =&gt; 'a</code> and the argument read has type `'a`.
