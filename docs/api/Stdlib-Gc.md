...
- `0x1000` Address space reservation changes. Default: 0\.
`max_overhead : int;`
...
let run_with_memory_limit = (limit: int, f: unit => 'a): 'a => {
  let limit_memory = () => {
    let mem = Gc.(quick_stat()).heap_words;
    if (mem / (1024 * 1024) > limit / (Sys.word_size / 8)) {
      raise(Out_of_memory);
    };
  };

  let alarm = Gc.create_alarm(limit_memory);
  Fun.protect(
    f,
    ~finally=() => {
...
```
```reasonml
let ramp_up: (unit => 'a) => ('a, suspended_collection_work);
```
...
If the user knows that at a certain point in the program the live memory consumption has been reduced by the corresponding amount \-- typically, because the memory allocated during `ramp_up` has become unused \-- then they should call [`ramp_down`](./#val-ramp_down) below to have the GC "resume" this collection work.
...
```
```reasonml
let ramp_down: suspended_collection_work => unit;
```
