...
```
```reasonml
let fold_left: f:('acc => char => 'acc) => init:'acc => string => 'acc;
```
...
```
```reasonml
let fold_right: f:(char => 'acc => 'acc) => string => init:'acc => 'acc;
```
...
```
```reasonml
let for_all: f:(char => bool) => string => bool;
```
...
```
```reasonml
let exists: f:(char => bool) => string => bool;
```
...
The function assumes the strings are UTF-8 encoded and uses [`Uchar.t`](./Stdlib-Uchar.md#type-t) for the notion of character. Decoding errors are replaced by [`Uchar.rep`](./Stdlib-Uchar.md#val-rep). Normalizing the strings to [NFC](https://unicode.org/glossary/#normalization_form_c) gives better results.
...
All strings are assumed to be UTF-8 encoded, decoding errors are replaced by [`Uchar.rep`](./Stdlib-Uchar.md#val-rep) characters.
