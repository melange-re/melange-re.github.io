...
```
```reasonml
let fold_left: f:('acc => char => 'acc) => init:'acc => bytes => 'acc;
```
...
```
```reasonml
let fold_right: f:(char => 'acc => 'acc) => bytes => init:'acc => 'acc;
```
...
let b = Bytes.make(size, ' ');
let update = (b, f, ()) =>
  Bytes.iteri((i, x) => Bytes.set(b, i, Char.chr(f(Char.code(x)))), b);
let d1 = Domain.spawn(update(b, x => x + 1));
let d2 = Domain.spawn(update(b, x => 2 * x + 1));
let () = {
  Domain.join(d1);
  Domain.join(d2);
};
