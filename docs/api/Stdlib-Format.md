...
**Warning**: Since [formatters](./#formatter) contain mutable state, it is not thread-safe to use the same formatter on multiple domains in parallel without synchronization. This may result in `Invalid_argument` being raised or an unspecified behavior.
...
```
```reasonml
let pp_print_substring: pos:int => len:int => formatter => string => unit;
```
```ocaml
val print_substring : pos:int -> len:int -> string -> unit
```
```reasonml
let print_substring: pos:int => len:int => string => unit;
```
...
```
```reasonml
let pp_print_bytes: formatter => bytes => unit;
```
```ocaml
...
```
```reasonml
let print_substring_as: pos:int => len:int => int => string => unit;
```
...
```
```reasonml
let pp_print_nothing: formatter => unit => unit;
```
Print nothing.
...
- the `out_width` function informs the formatting engine of the width of the substring as rendered on the output device. Explicit width information as provided by `@<n>`, [`pp_print_as`](./#val-pp_print_as) or [`pp_print_substring_as`](./#val-pp_print_substring_as) takes priority over this function. Moreover, the formatting engine evaluates the width of the string arguments of [`pp_print_string`](./#val-pp_print_string) and substring arguments of [`pp_print_substring`](./#val-pp_print_substring) as a whole. Consequently, `out_width` can be used to compute an approximative width for unicode substrings.
...
- field `out_width` is the number of unicode scalar values (see [`utf_8_scalar_width`](./#val-utf_8_scalar_width)) in the substring.
...
Reasonable defaults for functions `out_spaces`, `out_newline`, and `out_width` are respectively `out_funs.out_string (String.make n ' ') 0 n`, `out_funs.out_string "\n" 0 1` and [`utf_8_scalar_width`](./#val-utf_8_scalar_width).
...
```
```reasonml
let utf_8_scalar_width: string => pos:int => len:int => int;
```
`utf_8_scalar_width s ~pos ~len` is the number of unicode scalar values in the substring `String.sub s pos len`. Invalid byte sequences are implictly replaced by `U+FFFD` since this yields a better width approximation for other ascii-based encoding scheme like ISO-8859-15. This is the default `out_width` function since OCaml 5\.4.
...
```
```reasonml
let ascii_width: string => pos:int => len:int => int;
```
...
```
```reasonml
let synchronized_formatter_of_out_channel: 
  out_channel =>
  Domain.DLS.key(formatter);
```
...
```
```reasonml
let get_std_formatter: unit => formatter;
```
...
```
```reasonml
let get_err_formatter: unit => formatter;
```
...
```
```reasonml
let get_str_formatter: unit => formatter;
```
...
```
```reasonml
make_formatter(Stdlib.output_substring(oc), () => Stdlib.flush(oc));
```
...
```
```reasonml
let make_synchronized_formatter: 
  (string => int => int => unit) =>
  (unit => unit) =>
  Domain.DLS.key(formatter);
```
...
let format_text: 
  ...
  format6('a, 'b, 'c, 'd, 'e, 'f);
```
`format_text fmt` replaces spaces and newlines in the format string literal `fmt` with hint breaks or forced newlines:

- Blank lines (lines made only of spaces (`U+0020`)) are replaced by `"\@n"`.
- Sequences of spaces and a newline (`U+000A`) preceding a blank line are replaced by `"\@n"`: blank lines remain blank lines in the output.
- Remaining sequences made of `k` spaces and newlines are replaced by `"@;<0 k'>"` where `k' = max 1 k` is at least `1`.
...
```
Same as [`fprintf`](./#val-fprintf), except the formatter is the last argument. `dprintf "..." a b c` is a function of type <code class="text-ocaml">formatter -&gt; unit</code><code class="text-reasonml">formatter =&gt; unit</code> which can be given to a format specifier `%t`.
...
We have a list `l` of pairs <code class="text-ocaml">(int * bool)</code><code class="text-reasonml">(int, bool)</code>, which the toplevel prints for us:
...
- `pp_pair` prints a pair <code class="text-ocaml">bool*int</code><code class="text-reasonml">(bool, int)</code> surrounded in "(" ")". It takes a formatter (into which formatting happens), and the pair itself. When printing is done it returns `()`.
...
- "%a" is a formatting instruction, like "%d" or "%s" for `printf`. However, where "%d" prints an integer and "%s" prints a string, "%a" takes a printer (of type <code class="text-ocaml">Format.formatter -&gt; 'a -&gt; unit</code><code class="text-reasonml">(Format.formatter, 'a) =&gt; unit</code>) and a value (of type `'a`) and applies the printer to the value. This is key to compositionality of printers.
...
```
```reasonml
external pp_point: (Format.formatter, point) => unit = ;

external pp_rectangle: (Format.formatter, rectangle) => unit = ;
```
